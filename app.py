...
def parameters():
    ...
    if age < 18:
        ...
        return jsonify(messages = "Welcome " + name + ", you are " + str(age) + " years old.")
...
def register():
    ...
    if test:
        ...
        db.session.add(user)
        db.session.commit()
        return jsonify(message = 'Thank you for registering. Please login.'), 201
...
def add_planet():
    ...
    if test:
        return jsonify(message = 'That planet is already registered.'), 409
    else:
        ...
        db.session.add(Planet(planet_name=planet_name,
